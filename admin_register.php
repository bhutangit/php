...
<?php
    if(isset($_REQUEST['submit']))
    {
        ...
        $query = "INSERT INTO `register`(`name`, `email`, `password`, `contact`) VALUES ('$name','$email','$password','$contact')";
        $result = mysqli_query($conn,$query);
        if($result>0)
        {
            // echo "Record Inserted";
            echo "<script>window.location.assign('register.php?msg=Record Inserted&status=success')</script>";
